...
from instaljsonhelpers import dict_funs_to_list, trace_dicts_from_file
...
def instal_state_facts(args):
    ...
    for f in args.fact_files:
        ...
        if ext == ".iaf":
            initlist += instal_fact_iaf(f)
        if ext == ".json":
            initlist += instal_fact_json(f)
...
def instal_fact_json(json_file):
    jsons = trace_dicts_from_file(json_file)
    if len(jsons) == 0: return []
