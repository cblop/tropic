...
(defn generates [trope tropes]
  (let [params (get-all-params trope)
        ...
        p (println "Filter:")
        p (println subs)
        p (println (prn-str (map (fn [x] (first (:events (:subtrope x)))) subs)))
        p (println (prn-str (extract-ors (remove :obligation (:events trope)))))
        events (remove #(in? (map (fn [x] (:trigger x)) subs) %) (extract-ors (remove :obligation (:events trope))))
